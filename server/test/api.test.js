...
describe('Testing Node Server User API\'s', () => {
  describe('Testing Get all users API', () => {
    test('It should response the GET method', async () => {
      const response = await request(app).get(ALL_USER_ENDPOINT)
  ...
  describe('Testing removeAll Users', () => {
    test('It should Delete All the Users', async () => {
      const response = await request(app).post(DELETE_ALL_USER_ENDPOINT)
      expect(response.statusCode).toBe(200)
      expect(response.body.data).toBe('Done')
      const _response = await request(app).get(ALL_USER_ENDPOINT)
  ...
  describe('Testing Add A new User', () => {
    test('It should add a new User', async () => {
      const response = await request(app).post(ADD_USER_ENDPOINT).send(user)
      expect(response.statusCode).toBe(200)
    ...
    describe('Getting the added new User', () => {
      test('It should get a User and match the firstName', async () => {
        const response = await request(app).post(`${UPDATE_USER_ENDPOINT}/${user._id}`, {hobbies: 'Fishing, Talking'})
        expect(response.statusCode).toBe(200)
        let {data} = response.body
        expect(data.length).toBe(1);
        ([data] = data)
    ...
    describe('Deleting the added new User', () => {
      test('It should get a User and match the firstName', async () => {
        const response = await request(app).post(`${DELETE_USER_ENDPOINT}/${user._id}`)
        expect(response.statusCode).toBe(200)
      })
    })
  })
